<template>
    <div v-if="transactionStore.order">
        <VRow>
            <VCol cols="12">
                <h3>Pago de compra #{{ numTransaction }}</h3>
            </VCol>
            <VCol cols="12">
                <div
                ...
                >
                    ...
                    <div class="my-auto">
                        ...
                        <span class="text-uppercase cursor-pointer text-decoration-underline" @click="openModal = true">click aqui</span>
                    </div>
                </div>
            </VCol>
            <VCol cols="12">
                ...
                >
                    ...
                    <div class="my-auto">
                        No recibimos transferencias virtuales a menos que tengas
                        la verificacion de identidad aprobada (KYC), la cuenta
                        desde la que transfieres debe estar a nombre tuyo de lo
                        contrario la transaccion sera rechaza y se cobrara un
                        costo administrativo del 1.4% del total de la
                        transaccion.
                    </div>
                </div>
            </VCol>
        ...
        <VRow>
            ...
            <VCol cols="6" class="my-6 h-100"
            style="height: 200px;"
            >
                <div class="v-text-field__slot h-100">
                    <div>Logo</div>
                    <div class="text-left mx-5 my-6">
                        <span
                            >Titular:
                            {{
                                transactionStore.order.relationships
                                    ?.account_delivery.attributes.beneficiary
                            }}</span
                        ><br />
                        <span
                            >Número de Cuenta:
                            {{
                                transactionStore.order.relationships
                                    ?.account_delivery.attributes.account_number
                            }}</span
                        ><br />
                        <span
                            >Banco:
                            {{
                                ...
                                ).relationships?.bank?.attributes.name ?? ""
                            }}</span
                        ><br />
                        <span
                            >Tipo de Cuenta:
                            {{
                                (
                                    transactionStore.order.relationships
                                        ?.account_delivery as BankAccount
                                ).relationships?.typeAccountBank?.attributes
                                    .name ?? ""
                            }}</span
                        ><br />
                    </div>
        ...
        <VRow>
            <div class="text-table mt-7">
                ...
                para enviarnos tu comprobante de lo contrario será cancelada, y no podrás crear
                una nueva orden hasta dentro de
                <span class="text-error text-h6">30 minutos</span>
            </div>
            <div class="text-center w-100 mt-4">
                <VBtnDanger @click="alerta" style="background-color: #EF5DA8!important;">
...
import UploadImageComponent from "../../Kyc/components/UploadImageComponent.vue";
import { ref } from "vue";
